...
class ModeExtraction:
    ...
    def __init__(self, user="codinguser", repo="gnucash-android"):
        ...
        self.difftextpredealfile = open(os.path.join(self.pyg.diffdirpath3,"diff.txt"), 'w')

        self.diffDR=open(os.path.join(self.pyg.diffdirpath3,"diffDR.txt"), 'w')
    ...
    def getDr95(self):
        ...
        for data in coll.find():
            # print data["DR0"]
            for key,value in data["DR0"].items():
                # print key,value
                thedata[key]={}
                for k,v in value.items():
                   if float(v)>0.90:
                       thedata[key][k]=data["diff"][key][k]
                       # print key,k
                       # print data["diff"][key][k]
        ...
        for key in thedata.keys():
            ...
            for k in thedata[key]:
                if key.endswith("java"):
                    ...
                    other += 1
                filedata[key]=filedata.get(key,0)+1
                self.diffDR.write(str(key)+","+str(k)+"\n")
                for item in thedata[key][k]:
                    self.diffDR.write(item+"\n")
                self.diffDR.write("\n")
        print "java", "xml", "other"
        print java,xml,other
        print filedata
    ...
    def getalldiffinfo(self):
        ...
        for data in coll.find():
            ...
            for key, value in data["diff"].items():
                drdiff[key]={}
                for k, v in value.items():
                    drdiff[key][k] = str(a.getDR1(v))
            coll.update_one({"_id":data["_id"]}, {'$set':{ "DR0":drdiff}})
                # yield key,value
    def getDR1(self,diff=[]):
        a=""
        b=""
        for d in diff:
            if d.startswith("+"):
