...
class DealMongo:
    ...
    def deal(self,label_type):
        ...
        for result in self.get_isuue(label_type):
            i+=1
            if coll.find({"number":result["number"]}):
                print "find"
            pull={"issue":{},"commits":[],"number":result["number"],"type":label_type}
            difffile = {}
            rawfile={}
            ...
            commits=[]
            '爬取pull of this issue'
            self.logger.info(str(i)+", 爬取pull of this issue")
            ...
            commit = {
                      ...
                      "message": [pull_commit["commit"]["message"]  for pull_commit in pull_commits],
                      "commit_changes":(0,0,0)
                      }
            j=0
            for pull_commit in pull_commits:
                j+=1
                '爬取each commit of commits'
                self.logger.info(str(i)+", " +str(j)+u", 爬取each commit of commits")
                ...
                # print pull_commit["commit"]["comment_count"]
                if pull_commit["commit"]["comment_count"] > 0:
                    '爬取pull  comments'
